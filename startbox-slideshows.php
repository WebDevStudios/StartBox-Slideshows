...
if ( ! function_exists( 'sb_slideshow_init' )  ) {
...
function sb_slideshow_save( $post_id ) {
	...
	if ( isset( $_POST['sb_ssorri'] ) )
		update_post_meta($post_id, 'ssorri', sanitize_text_field( $_POST['sb_ssorri'] ) );
	
	if ( isset( $_POST['sb_effect'] ) ) 
		$sb_effect = array_map( 'sanitize_text_field', $_POST['sb_effect'] );
		update_post_meta( $post_id, 'effect', $sb_effect );
	
	//TODO:  If option was saved and then both unchecked the option is not saved
	if ( isset( $_POST['sb_control'] ) && ! empty( $_POST['sb_control'] ) )
		$sb_control = array_map( 'sanitize_text_field', $_POST['sb_control'] );
		update_post_meta( $post_id, 'control', $sb_control );
	
	update_post_meta( $post_id, 'pause', (is_numeric( $_POST['sb_pause'] ) ? absint( $_POST['sb_pause'] ) : $sb_slideshow_interface['pause']) );
	update_post_meta( $post_id, 'opacity', (is_numeric( $_POST['sb_opacity'] ) ? sanitize_text_field( $_POST['sb_opacity'] ) : $sb_slideshow_interface['opacity']) );
	update_post_meta( $post_id, 'size', sanitize_text_field( $_POST['sb_size'] ) );
	...
	//sanitize custom size values
	$size_custom = array_map( 'absint', $size_custom );
...
function sb_slideshow_template_redirect() {
	...
	if( has_action( 'sb_slideshow_enqueue' ) ) { // allow default enqueues to be replaced
		...
		wp_enqueue_script( 'nivoslider', plugins_url( '/jquery-nivo/jquery.nivo.slider.js', __FILE__ ) );
		wp_enqueue_style( 'nivo_slider', plugins_url( '/jquery-nivo/css/nivo-slider.css', __FILE__ ) );
		wp_enqueue_style( 'nivo_custom', plugins_url( '/jquery-nivo/css/custom-nivo-slider.css', __FILE__ ) );
	}
}
